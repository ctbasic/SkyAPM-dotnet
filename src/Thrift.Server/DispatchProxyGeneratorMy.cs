...
namespace System.ReflectionMy
{
    ...
    //  1. This version is based on DispatchProxyMy from NET Native and CoreCLR, not RealProxy in Silverlight ServiceModel.
    //     There are several notable differences between them.
    //  2. Both DispatchProxyMy and RealProxy permit the caller to ask for a proxy specifying a pair of types:
    ...
    //       - DispatchProxyMy generates a proxy type that *derives* from the base type and implements all
    //         the interface type's interfaces.  This is true for both the CLR version in NET Native and this
    //         version for CoreCLR.
    //  3. DispatchProxyMy and RealProxy use different type hierarchies for the generated proxies:
    ...
    //       - DispatchProxyMy hierarchy is:
    //             proxyType : baseType (where baseType : DispatchProxyMy)
    //         The generated DispatchProxyMy proxy type does not need to generate implementation methods
    ...
    //     But the DispatchProxyMy design encourages the proxy type to *be* an DispatchProxyMy.  Therefore,
    //     the proxy's 'this' becomes the equivalent of RealProxy's backpointer to RealProxy, so we were
    //     able to remove an extraneous field and ctor arg from the DispatchProxyMy proxies.
    //
    internal static class DispatchProxyGeneratorMy
    {
        ...
        // Proxy instances are not cached.  Their lifetime is entirely owned by the caller of DispatchProxyMy.Create.
        ...
        private static readonly MethodInfo s_dispatchProxyInvokeMethod = typeof(DispatchProxyMy).GetTypeInfo().GetDeclaredMethod("Invoke");
        ...
        internal static object CreateProxyInstance(Type baseType, Type interfaceType)
        {
            ...
            return Activator.CreateInstance(proxiedType, (Action<object[]>)DispatchProxyGeneratorMy.Invoke);
        }
        ...
        // to the (abstract) DispatchProxyMy.Invoke() method.
        private static void Invoke(object[] args)
        {
            ...
            // Call (protected method) DispatchProxyMy.Invoke()
            try
            {
                Debug.Assert(s_dispatchProxyInvokeMethod != null);
                object returnValue = s_dispatchProxyInvokeMethod.Invoke(packed.DispatchProxyMy,
        ...
        private class PackedArgs
        {
            ...
            internal DispatchProxyMy DispatchProxyMy { get { return (DispatchProxyMy)_args[DispatchProxyPosition]; } }
        ...
        private class ProxyBuilder
        {
            ...
            private MethodBuilder AddMethodImpl(MethodInfo mi)
            {
                ...
                il.Emit(OpCodes.Ldarg_0);
                packedArr.EndSet(typeof(DispatchProxyMy));
